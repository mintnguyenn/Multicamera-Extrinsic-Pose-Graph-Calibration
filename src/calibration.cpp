...
void ExtrinsicCalibration::runThreads(void)
{
  ...
  threads_.push_back(std::thread(&ExtrinsicCalibrationInterface::thread, this)); // Create the thread and push it to our vector of threads
}
...
void ExtrinsicCalibration::thread()
{
  ...
  while (running_)
  {
    ...
    {
      cv::solvePnP(objPoints, imgPoints, instrinsic, distCoeffs, rvec, tvec, false, 0);
